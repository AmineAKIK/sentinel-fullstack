...
export async function getBoardAccess(): Promise<BoardAccessResponse> {
  return api.get<BoardAccessResponse>('/api/board/me');
}
...
export async function getBoardData(): Promise<WorkshopBoardData> {
  return api.get<WorkshopBoardData>('/api/board/data');
}
