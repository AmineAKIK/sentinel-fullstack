...
describe('OPERATOR permissions', () => {
  it('can REQUEST_EDIT on their own active incident', () => {
    expect(canPerform('OPERATOR', 'REQUEST_EDIT', incident({ status: 'OPEN' }), ACTOR_ID)).toBe(
      true
    );
    expect(canPerform('OPERATOR', 'REQUEST_EDIT', incident({ status: 'PENDING' }), ACTOR_ID)).toBe(
      true
    );
  });
  ...
  it('cannot REQUEST_EDIT on a closed/canceled incident', () => {
    expect(canPerform('OPERATOR', 'REQUEST_EDIT', incident({ status: 'CLOSED' }), ACTOR_ID)).toBe(
      false
    );
    expect(canPerform('OPERATOR', 'REQUEST_EDIT', incident({ status: 'CANCELED' }), ACTOR_ID)).toBe(
      false
    );
  });
  ...
  it('can REQUEST_CANCEL on their own open, non-taken incident', () => {
    expect(
      canPerform(
        'OPERATOR',
        'REQUEST_CANCEL',
        incident({ status: 'OPEN', is_taken: false }),
        ACTOR_ID
      )
    ).toBe(true);
  });
  ...
  it("cannot REQUEST_CANCEL on another operator's incident", () => {
    expect(
      canPerform('OPERATOR', 'REQUEST_CANCEL', incident({ user_id: 99, is_taken: false }), ACTOR_ID)
    ).toBe(false);
  });

  it('cannot REQUEST_CANCEL when incident is taken', () => {
    expect(canPerform('OPERATOR', 'REQUEST_CANCEL', incident({ is_taken: true }), ACTOR_ID)).toBe(
      false
    );
  });

  it('cannot REQUEST_CANCEL on a closed incident', () => {
    expect(canPerform('OPERATOR', 'REQUEST_CANCEL', incident({ status: 'CLOSED' }), ACTOR_ID)).toBe(
      false
    );
  });
...
describe('MAINTENANCE permissions', () => {
  ...
  it('can SET_PENDING when OPEN and taken', () => {
    expect(
      canPerform('MAINTENANCE', 'SET_PENDING', incident({ status: 'OPEN', is_taken: true }))
    ).toBe(true);
  });

  it('cannot SET_PENDING when not taken', () => {
    expect(
      canPerform('MAINTENANCE', 'SET_PENDING', incident({ status: 'OPEN', is_taken: false }))
    ).toBe(false);
  });
  ...
  it('cannot RESUME when OPEN', () => {
    expect(canPerform('MAINTENANCE', 'RESUME', incident({ status: 'OPEN', is_taken: true }))).toBe(
      false
    );
  });
  ...
  it('can DIRECT_EDIT on active non-taken incident', () => {
    expect(
      canPerform('MAINTENANCE', 'DIRECT_EDIT', incident({ status: 'OPEN', is_taken: false }))
    ).toBe(true);
  });
  ...
  it('can CANCEL on active non-taken incident', () => {
    expect(canPerform('MAINTENANCE', 'CANCEL', incident({ status: 'OPEN', is_taken: false }))).toBe(
      true
    );
  });

  it('cannot APPROVE_EDIT, REJECT_EDIT, INVALIDATE_CLOSED', () => {
...
describe('RESPONSABLE permissions', () => {
  it('can APPROVE_EDIT and REJECT_EDIT only when an edit request exists', () => {
    expect(
      canPerform('RESPONSABLE', 'APPROVE_EDIT', incident({ edit_request: { comment: 'x' } }))
    ).toBe(true);
    expect(
      canPerform('RESPONSABLE', 'REJECT_EDIT', incident({ edit_request: { comment: 'x' } }))
    ).toBe(true);
  ...
  it('can APPROVE_CANCEL when cancel_request is true', () => {
    expect(canPerform('RESPONSABLE', 'APPROVE_CANCEL', incident({ cancel_request: true }))).toBe(
      true
    );
  });

  it('cannot APPROVE_CANCEL when cancel_request is false', () => {
    expect(canPerform('RESPONSABLE', 'APPROVE_CANCEL', incident({ cancel_request: false }))).toBe(
      false
    );
  });

  it('can REJECT_CANCEL only when cancel_request is true', () => {
    expect(canPerform('RESPONSABLE', 'REJECT_CANCEL', incident({ cancel_request: true }))).toBe(
      true
    );
    expect(canPerform('RESPONSABLE', 'REJECT_CANCEL', incident())).toBe(false);
  });

  it('can SET_PRIORITY, RESPONSIBLE_COMMENT on active incidents', () => {
    const inc = incident();
  ...
  it('can INVALIDATE_CLOSED only when status is CLOSED', () => {
    expect(canPerform('RESPONSABLE', 'INVALIDATE_CLOSED', incident({ status: 'CLOSED' }))).toBe(
      true
    );
    expect(canPerform('RESPONSABLE', 'INVALIDATE_CLOSED', incident({ status: 'OPEN' }))).toBe(
      false
    );
  });
  ...
  it('can DIRECT_EDIT on active non-taken incident', () => {
    expect(
      canPerform('RESPONSABLE', 'DIRECT_EDIT', incident({ status: 'OPEN', is_taken: false }))
    ).toBe(true);
  });
