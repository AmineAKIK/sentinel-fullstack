...
| 11   | Documentation et candidate                           | `docs: synchronize the readiness matrix with measured facts` (046f021)            | FAIT (montée 048→050 sur base figée validée sur PG réel). Les totaux courants et vérifiés font autorité dans la section « Faits mesurés » en tête du document. |
...
| 11+  | Contrôles locaux (équivalents CI) + montée RC2 figée | `test+docs: run the CI-equivalent local controls and tie 048→050 to the RC2 fixture` (8b3d7c0) | FAIT (cf. §4.1 : builds, audits, ShellCheck, conteneurs, préflight, `.env`, backup/restore, migrations, 048→049→050 RC2 ; 0 résidu Docker)                                                                      |
