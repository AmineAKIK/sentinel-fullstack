...
| **R4-05** | **P0** | Plan RC4 §8.1–8.2, §12 lots 4–5, §14.5 | Rouge du lot 0 : `useMutationRunner` n'avait aucun import de production. Lot 4 : les 24 mutations Atelier consomment désormais un contrat partagé ; les autres surfaces restent au lot 5. | Contrat shell exact de recherche des consommateurs de production, depuis `frontend/` ; matrice rouge ciblée et tests d'architecture, voir §6 | Rouge code `1`, message exact conservé en §6 ; vert Atelier : quatre orchestrateurs de production, aucun ancien runner Atelier et 102 tests ciblés verts | `frontend/src/components/ui/MutationFeedback.tsx` ; hooks/actions et modales Atelier ; pages Administration/Auth/Board/Support au lot 5 ; `docs/rc4-mutation-inventory.md` | Conserver un unique contrat léger et étendre son adoption aux autres surfaces au lot 5 sans réintroduire de machine locale. | Tests contractuels communs prêt/confirmation/pending/succès/échec/récupération ; course, démontage, timers ; anti-double ; focus ; erreurs réseau/métier ; E2E réels par famille. | Lot 4 : `102/102` ciblés, `534/534` frontend, `48/48` Chromium ; lot 5 : `PENDING_EXECUTION[R4-05:GREEN_NON_WORKSHOP]` | Doubles messages, verrouillage incohérent, perte de focus, abstraction trop large, lignes non-Atelier oubliées. | `PARTIALLY_VERIFIED_LOT4` |
| **R4-06** | **P0** | Plan RC4 §8.1, §8.3–8.4, §14.3–14.5 | Inventaire initial : `61` lignes, `16 GAP`, `11` exceptions. Lot 4 : les 24 lignes Atelier sont `COVERED` ; restent `14 PARTIAL`, `13 GAP` et `10 EXCEPTION_TO_REVIEW` hors Atelier. Les lacunes supplémentaires restent figées sous R4-11 en §7. | Chaque interaction corrigée au lot 4 possède un rouge ciblé exécuté avant correction ; inventaire détaillé et registre §6 | Matrice rouge : `29 failed / 49 passed` sur six fichiers, plus rouges ciblés par famille ; vert : 24 lignes Atelier couvertes avec unité, intégration UI et navigateur réel | Toutes les lignes de `docs/rc4-mutation-inventory.md`; Administration, Support, Auth et Board restent au lot 5 | Étendre les cinq états hors Atelier ; conserver anti-double, message sûr, saisie/modale/focus ; confirmations accessibles pour toute action définitive. | Par famille : erreur métier/réseau, conservation byte-identique, bouton réutilisable, aucune clé brute, double clic = un appel, succès exact et focus restauré ; E2E réels. | Lot 4 : Atelier `24 COVERED` ; lot 5 : `PENDING_EXECUTION[R4-06:NON_WORKSHOP]` | Action irréversible sans confirmation, fermeture optimiste, champs perdus, erreur technique visible, double envoi, confirmations empilées. | `PARTIALLY_VERIFIED_LOT4` |
...
| --- | --- | --- |
| R4-06 | 24 lignes Atelier couvertes au lot 4 ; surfaces non-Atelier inchangées | `PENDING_EXECUTION[R4-06:NON_WORKSHOP]` |
